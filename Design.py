...
for i in range(500, 600, 1):
    ...
    print('eps为%.3f时FMI评价分值为：%f' % (i * 0.001, score))
...
for eps in eps_values:
    dbscan = DBSCAN(eps=eps, min_samples=4).fit(wine_trainPCA)
    silhouette_avg = metrics.silhouette_score(wine_trainPCA, dbscan.labels_, metric='euclidean')
    silhouette_score.append(silhouette_avg)

plt.figure(figsize=(9, 6))
plt.plot(eps_values, silhouette_score, marker='o')
plt.xlabel('eps')
plt.ylabel('轮廓系数')
plt.show()
